<script lang="ts">
	...
	import { cacheModel } from '../lib/utils';
	...
	async function main(): Promise<void> {
		...
		try {
			...
			resultPlaceholder =
				'Error loading OCR model: ' + (err instanceof Error ? err.message : String(err));
			return;
		}
	...
	async function infer(): Promise<void> {
		...
		try {
			...
			if (result?.parragraphs && result.parragraphs.length > 0) {
				resultText = result.parragraphs.map((line: { text: string }) => line.text).join('\n');
				resultDisabled = false;
				saveDisabled = false;
			} else {
				resultPlaceholder = 'No text detected.';
				resultDisabled = false;
			}
		} catch (e) {
			console.error('OCR error:', e);
			resultText = 'OCR failed: ' + (e instanceof Error ? e.message : String(e));
			resultDisabled = false;
		}
	}
...
<main>
	<section id="camera-wrapper" class="camera-wrapper">
		...
		<a href="#image-process">
			<img id="preview" class="preview" src={previewSrc} alt="Preview of camera capture" />
		</a>
	</section>
	...
	<section id="image-process" class="image-process">
		...
		></textarea>
		<footer>
			<button id="infer" class="infer" on:click={infer} disabled={inferDisabled}>
				🤖 Process Image
			</button>
